  ...
  const content = destinations.map((dest, index)=>{
    return (
      <FormGroup validationState={props.getValidationState(destsValid[index])}>
        <Col sm={2} smOffset={2} componentClass={ControlLabel}>{"Destination Stop " + (index+1)}</Col>
        <Col sm={3}><FormControl id={"destination_"+index} type="text" value={dest} onChange={(e)=>props.onChange(e, index)} /></Col>
        <Col sm={2}><FormControl id={"destination_towns_"+index} type="text" value={destinationTowns[index]} onChange={(e)=>props.onChange(e, index)} /><FormControl.Feedback /></Col>
...
function BudgetingEntity(props) {
  ...
  return (
    <FormGroup validationState={props.getValidationState(bEntityValid)}>
      <Col sm={2} smOffset={2} componentClass={ControlLabel} >Budgeting Entity: </Col>
      ...
      </FormControl><FormControl.Feedback /></Col>
      {pCode}
    </FormGroup>
  );
}
...
export default class RequestTrip extends React.Component {
  ...
  validateForm(){
    this.setState({formValid: this.state.ttypeValid && this.state.tripDtValid && this.state.tripTmValid && this.state.prpsValid && this.state.budgetingEntityValid });
  }
  ...
  render() {
    const fieldNames=['Trip Type', 'Trip Date', 'Trip Time', 'Trip Purpose', 'Budgeting Entitiy', 'Further Remarks'];

    return (
      <Form horizontal onSubmit={this.handleSubmit}>
        <FormGroup controlId="tripID">
          <Col sm={2} smOffset={2} componentClass={ControlLabel}>Trip Number:</Col>
          <Col sm={3}><FormControl type="text" value={this.state.tripID} readOnly='true' /></Col>
        </FormGroup>

        <FormGroup controlId="tripRequester">
          <Col sm={2} smOffset={2} componentClass={ControlLabel}>Requester:</Col>
          <Col sm={3}><FormControl type="text" value={this.state.rqstrID} readOnly='true' /></Col>
        </FormGroup>
        ...
        <FormGroup controlId="tripDate" validationState={this.getValidationState(this.state.tripDtValid)}>
          ...
          <Col sm={3}><FormControl type="date" value={this.state.tripDate} onChange={this.handleChange} /><FormControl.Feedback /></Col>
        </FormGroup>
        ...
        <FormGroup controlId="tripTime" validationState={this.getValidationState(this.state.tripTmValid)}>
          ...
          <Col sm={3}><FormControl type="time" value={this.state.tripTime} onChange={this.handleChange} /><FormControl.Feedback /></Col>
        </FormGroup>
        ...
        <FormGroup controlId="tripPurpose" validationState={this.getValidationState(this.state.prpsValid)}>
          ...
          <Col sm={5}><FormControl type="text" value={this.state.tripPurpose} onChange={this.handleChange} /><FormControl.Feedback /></Col>
        </FormGroup>
        ...
        <FormGroup controlId="fthrRemarks">
          <Col sm={5} smOffset={4}><FormControl type="textarea" rows={5} value={this.state.fthrRemarks} readOnly={!(this.state.remarksAdded)} onChange={this.handleChange} /></Col>
        </FormGroup>
